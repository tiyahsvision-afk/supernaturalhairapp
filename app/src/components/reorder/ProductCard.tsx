    ...
    <GlowCard className={`relative ${selected ? '!border-fuchsia-300' : ''}`}>
      <button
        ...
        className={`absolute right-4 top-4 grid h-6 w-6 place-items-center rounded-full border text-xs ${
          selected ? 'border-fuchsia-300 bg-fuchsia-400 text-ink-950' : 'border-ink-900/25 text-transparent'
        }`}
      ...
      <h3 className="mt-3 font-display text-sm font-bold text-ink-900">{product.title}</h3>
      <p className="mt-1 line-clamp-2 text-xs text-ink-900/55">{product.description}</p>
      <div className="mt-3 flex items-center justify-between">
        <span className="text-sm font-semibold text-lavender-600">${product.price.toFixed(2)}</span>
        <a
          href={reorderSingleUrl(product.id)}
          target="_blank"
          rel="noreferrer"
          className="rounded-full border border-ink-900/20 px-3 py-1.5 text-xs font-semibold text-ink-900 hover:bg-ink-900/10"
        >
          Reorder →
        </a>
      </div>
