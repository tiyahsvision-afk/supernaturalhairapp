...
export default function PointsHero() {
  ...
  return (
    <GlowCard className="bg-gradient-to-br from-fuchsia-500/10 via-transparent to-sky-500/10">
      <p className="text-xs font-semibold uppercase tracking-[0.25em] text-fuchsia-600">Your balance</p>
      <p className="mt-2 font-display text-5xl font-extrabold text-gradient">{points}</p>
      <p className="text-sm text-ink-900/50">points ready to redeem on your next order</p>

      <div className="mt-6 grid gap-2 sm:grid-cols-2">
        {EARN_ACTIONS.map((a) => (
          <div key={a.label} className="flex items-center justify-between rounded-xl border border-ink-900/10 bg-ink-900/[0.03] px-3 py-2.5">
            <span className="flex items-center gap-2 text-sm text-ink-900/75">
              <span>{a.icon}</span>
              {a.label}
            </span>
            <span className="text-xs font-semibold text-fuchsia-600">+{a.points}</span>
          </div>
        ))}
      </div>
    </GlowCard>
