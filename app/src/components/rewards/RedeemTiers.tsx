...
export default function RedeemTiers() {
  ...
  return (
    <GlowCard>
      <h3 className="font-display text-lg font-bold text-white">Redeem your points</h3>
      <div className="mt-4 space-y-2.5">
        {TIERS.map((tier) => {
          ...
          return (
            ...
            >
              <div>
                <p className="text-sm font-semibold text-white">{tier.reward}</p>
                <p className="text-xs text-white/45">{tier.points} points</p>
              </div>
              <button
                onClick={() => handleRedeem(tier.points, tier.reward)}
                disabled={!canRedeem}
                className="rounded-full border border-white/20 px-4 py-1.5 text-xs font-semibold text-white disabled:opacity-30"
              >
                Redeem
              </button>
            </div>
