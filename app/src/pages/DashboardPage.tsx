...
export default function DashboardPage() {
  ...
  return (
    <PageShell>
      ...
      <p className="mt-2 text-ink-900/60">Here's where your journey stands today.</p>

      <div className="mt-8 grid gap-6 lg:grid-cols-[1.3fr,1fr]">
        <div className="space-y-6">
          <GlowCard>
            <div className="flex items-center justify-between">
              <h3 className="font-display text-lg font-bold text-ink-900">Today</h3>
              {hairstyleToday && (
                <span className="rounded-full bg-sky-400/10 px-3 py-1 text-xs font-semibold text-sky-600">
          ...
          {latestConsultation ? (
            <GlowCard>
              <p className="text-xs font-semibold uppercase tracking-[0.2em] text-fuchsia-600">Your active plan</p>
              <p className="mt-2 text-sm text-ink-900/70">{latestConsultation.summary}</p>
              <Link to="/consultation" className="mt-3 inline-block text-sm font-semibold text-fuchsia-600 hover:underline">
                View or update your consultation →
              </Link>
            </GlowCard>
          ) : (
            <GlowCard className="bg-gradient-to-br from-fuchsia-500/10 to-sky-500/10">
              <p className="font-display text-lg font-bold text-ink-900">You haven't taken your consultation yet</p>
              <p className="mt-1 text-sm text-ink-900/60">Get a personalized plan for your Journey Kit in under two minutes.</p>
              <Link
                to="/consultation"
        ...
        <div className="space-y-6">
          <GlowCard className="text-center">
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-fuchsia-600">Points</p>
            <p className="mt-1 font-display text-4xl font-extrabold text-gradient">{points}</p>
            <Link to="/rewards" className="mt-2 inline-block text-sm text-ink-900/60 hover:text-ink-900">
              View rewards →
            </Link>
          </GlowCard>
          <NotificationToggle />
        ...
        {QUICK_LINKS.map((l) => (
          <Link key={l.to} to={l.to}>
            <GlowCard className="flex flex-col items-center gap-2 !p-4 text-center transition-transform hover:-translate-y-1">
              ...
              <span className="text-xs font-semibold text-ink-900/80">{l.label}</span>
            </GlowCard>
          </Link>
        ))}
      </div>
